...
class LeRobotEpisodeLoader:
    ...
    def configure_nvc_gop_store(self, store_id: int, capacity: int) -> None:
        ...
        The actual attachment is lazy so each DataLoader worker opens its own
        process-local handle, regardless of multiprocessing context.
        """
    ...
    def close_nvc_gop_worker_resources(self) -> None:
        ...
        if self._nvc_gop_decoder is not None:
            # GetGOPList caching owns native ``gs_*`` SHM; release it explicitly.
            try:
    ...
    def materialize_nvc_gop_request(self, request: NvcGopRequest) -> NvcGopRequest:
        ...
        for video_path, frame_ids in zip(request.video_paths, request.frame_ids):
            ...
            for frame_id in frame_ids:
                ...
                if ref is None:
                    # SharedGopStore is only a hand-off queue; the per-file cache
                    # avoids demuxing the same GOP again for adjacent steps.
    ...
    def load_episode_metadata(self, episode_index: int) -> pd.DataFrame:
