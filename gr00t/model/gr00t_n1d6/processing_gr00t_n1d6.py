...
class Gr00tN1d6Processor(BaseProcessor):
    ...
    def transform_decoded_vlm_images(
        self,
        decoded_samples: list[torch.Tensor],
    ) -> list[torch.Tensor]:
        """Apply only image transforms that consume decoded CUDA tensors."""

        return [self._transform_decoded_images(images) for images in decoded_samples]

    def collate_transformed_vlm_inputs(
        self,
        transformed_samples: list[torch.Tensor],
        languages: list[str],
        device: torch.device,
    ) -> dict[str, Any]:
        """Run the unchanged Eagle processor on already transformed images."""

        if len(transformed_samples) != len(languages):
            raise ValueError("transformed_samples and languages must have equal lengths")

        text_list = []
        image_inputs = []
        for transformed, language in zip(transformed_samples, languages):
            conversation = [
                {
                    "role": "user",
                    "content": [
